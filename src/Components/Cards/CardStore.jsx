		...
		fetch(`/api/store/purchase/ram/${amount}`, {
			...
			.then(json => {
				if (json.success) return MySwal.fire({
					icon: 'success',
					title: 'Sucesso!',
					text: `Você comprou ${amount}MB de RAM!`,
				}).then(() => {
		...
		fetch(`/api/store/purchase/cpu/${amount}`, {
			...
			.then(json => {
				if (json.success) return MySwal.fire({
					icon: 'success',
					title: 'Sucesso!',
					text: `Você comprou ${amount}% de CPU!`,
				}).then(() => {
		...
		fetch(`/api/store/purchase/disk/${amount}`, {
			...
			.then(json => {
				if (json.success) return MySwal.fire({
					icon: 'success',
					title: 'Success!',
					text: `Você comprou ${amount}MB de SSD!`,
				}).then(() => {
                    ...
                    <div className="card">
                        <div className="card-header border-0">
                            <div className="row align-items-center">
                                <div className="col">
                                    <h3 className="mb-0">Comprar Recursos</h3>
                                </div>
                            </div>
                        </div>
                        <div className="card-body">
							{isLoading ? <p className="text-center"><b>Carregando</b></p>
				   :
                            <form>
                                <div className="form-group">
                                    <label htmlFor="ram_amount">1MB RAM custa: {ramPrice} moedas</label>
                                    <input type="text" className="form-control form-control-alternative"
                                        name="ram_amount" id="ram_amount" placeholder=" " required/>
                                </div>
                                <button onClick={purchaseRam} type="button" className="btn btn-outline-default">Comprar</button>
                            </form> }
                            <hr/>
							{isLoading ? <p className="text-center"><b>Carregando</b></p>
				   :
                            <form>
                                <div className="form-group">
                                    <label htmlFor="disk_amount">1MB SSD custa: {diskPrice} moedas</label>
                                    <input type="text" className="form-control form-control-alternative"
                                        name="disk_amount" id="disk_amount" placeholder=" " required/>
                                </div>
                                <button onClick={purchaseDisk} type="button" className="btn btn-outline-default">Comprar</button>
                            </form> }
                            <hr/>
							{isLoading ? <p className="text-center"><b>Carregando</b></p>
				   :
                            <form>
                                <div className="form-group">
                                    <label htmlFor="cpu_amount">1% CPU custa: {cpuPrice} moedas</label>
                                    <input type="text" className="form-control form-control-alternative"
                                        name="cpu_amount" id="cpu_amount" placeholder=" " required/>
                                </div>
                                <button onClick={purchaseCpu} type="button" className="btn btn-outline-default">Comprar</button>
                            </form> }
                            <hr/>
                        </div>
                    </div>
